import Image from "@/node_modules/next/image";
import Link from "@/node_modules/next/link";
import React from "react";
import { FOOTER_LINKS, FOOTER_CONTACT_INFO, SOCIALS } from "@/constants/index";
import { FooterLinksProps, FooterLinks, FooterContactLinks } from "@/helper/index";


const FooterColumn = ({ title, children }: FooterLinksProps) => {
  return (
    <div className="flex flex-col gap-5">
      <h4 className="bold-18 whitespace-nowrap">{title} </h4>
      {children}
    </div>
  );
};

const Footer = () => {
  return (
    <footer className="flexCenter mb-24">
      <div className="max-container padding-container flex w-ull flex-col gap-14">
        <div className="flex flex-col items-center justify-center gap-[10%] md:flex-row">
          <Link href="/" className="mb-10">
            <Image src="/hilink-logo.svg" alt="logo" width={74} height={29} />
          </Link>
          <div className="flex flex-wrap gap-10 sm:justify-between md:flex-1">
            {FOOTER_LINKS.map(
              ({ title, links }: FooterLinks, index: number) => (
                <FooterColumn title={title} key={index}>
                  <ul className="regular-14 flex flex-col gap-4 text-gray-30">
                    {links.map((link: string, index: number) => (
                      <Link href="/" key={index}>
                        {link}
                      </Link>
                    ))}
                  </ul>
                </FooterColumn>
              )
            )}
            <div className="flex flex-col gap-5">
              <FooterColumn title={FOOTER_CONTACT_INFO.title}>
                {FOOTER_CONTACT_INFO.links.map(
                  ({label,value}: FooterContactLinks, index: number) => (
                    <Link href="/" key={index} className="flex gap-4 md:flex-col lg:flex-row">
                      <p className="whitespace-nowrap">{label}:</p>
                      <p className="medium-14 whitespace-nowrap text-blue-70 ">{value}</p>
                    </Link>
                  )
                )}
              </FooterColumn>
            </div>
            <div className="flex flex-col gap-5">
              <FooterColumn title={SOCIALS.title}>
                <ul className="regular-14 flex gap-4 text-gray-30">
                  {SOCIALS.links.map((link: string , index: number) => (
                    <Link href="/" key={index}>
                      <Image src={link} alt="logo" width={24} height={24} />
                    </Link>
                  ))}
                </ul>
              </FooterColumn>
            </div>
          </div>
        </div>
        <div className="border bg-gray-20" />
        <p className="regular-14 w-full text-center text-gray-30">2024 Hilink | All rights reserved</p>
      </div>
    </footer>
  );
};

export default Footer;
